...
# make numeric input
# train_x = np.array(data.iloc[:, -1 * (28*2 + args.use_days) : -28*2]).sum(axis=0).reshape(1,-1)
train_x = np.array(data.iloc[:, -1 * (28 + args.use_days) : -28]).sum(axis=0).reshape(1,-1)
train_t = np.array(data.iloc[:,  -28 * 2:-28]).sum(axis=0).reshape(1,-1)
valid_x = np.array(data.iloc[:, -1 * (28 + args.use_days) : -28]).sum(axis=0).reshape(1,-1)
