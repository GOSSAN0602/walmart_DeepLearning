...
sales_diff = np.load(f'{args.INPUT_DIR}/../lv1/sales_diff.npy')
...
sales_minmax = pd.read_csv(f'{args.INPUT_DIR}/../lv1/min_max.csv')
...
# normalize sales diff
# normalize_factor = np.abs(sales_diff[:, -1 * (28 + args.use_days) : -28*2]).max(axis=1).reshape(2,1)*1.1
# sales_diff = sales_diff / normalize_factor
