...
price_data = pd.read_csv(f'{args.INPUT_DIR}/../lv1/price_fea.csv')
...
price_data.drop(price_data.columns[[1061,1426,1791]], axis=1)
...
mm.fit(train_x.T)
# cut outrange & padding last 28days
...
# merge sells & days
n_dyn_fea = 8
input_train_x = np.concatenate([train_x, train_days_x, train_price_x]).reshape(1,n_dyn_fea,args.use_days)
input_train_t = train_t.reshape(1,28)
input_valid_x = np.concatenate([valid_x, valid_days_x, valid_price_x]).reshape(1,n_dyn_fea,args.use_days)
input_valid_t = valid_t.reshape(1,28)
input_test_x = np.concatenate([test_x, test_days_x, test_price_x]).reshape(1, n_dyn_fea, args.use_days)
